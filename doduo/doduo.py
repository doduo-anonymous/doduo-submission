...
class DFColTypeTablewiseDataset(Dataset):
    def __init__(self,
        ...
        for i in range(len(df.columns)):
            data_list.append([1,  # Dummy table ID (fixed)
                              0,  # Dummy label ID (fixed)
                              " ".join([str(x) for x in df.iloc[:, i].dropna().tolist()])])
        self.df = pd.DataFrame(data_list, columns=["table_id", "label_ids", "data"])
        ...
        for i, (index, group_df) in enumerate(self.df.groupby("table_id")):
            token_ids_list = group_df["data"].apply(
                lambda x: tokenizer.encode(x, add_special_tokens=True,
                                           max_length=max_length + 2)).tolist()
            ...
            for cls_index in cls_index_list:
                assert token_ids[cls_index] == tokenizer.cls_token_id, "cls_indexes validation"
            cls_indexes = torch.LongTensor(cls_index_list).to(device)
            class_ids = torch.LongTensor(group_df["label_ids"].tolist()).to(device)
    ...
    def __getitem__(self, idx):
        return {"data": self.table_df.iloc[idx]["data_tensor"],
...
class DFColTypeColwiseDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {"data": self.df.iloc[idx]["data_tensor"],
...
class Doduo:
    def __init__(self,
        ...
        ## Load models
        coltype_model_path = os.path.join(basedir, "model/turlturl-re-colpair_mosato_bert_bert-base-uncased-bs16-ml-16__turl-1.00_turl-re-1.00=turl_best_micro_f1.pt")
        colrel_model_path = os.path.join(basedir, "model/turlturl-re-colpair_mosato_bert_bert-base-uncased-bs16-ml-16__turl-1.00_turl-re-1.00=turl-re_best_micro_f1.pt")

        ## Load mlb
        with open(os.path.join(basedir, "data/turl_coltype_mlb.pickle"), "rb") as fin:
            self.coltype_mlb = pickle.load(fin)
        with open(os.path.join(basedir, "data/turl_colrel_mlb.pickle"), "rb") as fin:
        ...
        self.coltype_model.load_state_dict(torch.load(coltype_model_path,
        ...
        if self.args.colpair:
            ...
            self.colrel_model.bert.pooler = BertMultiPairPooler(config).to(self.device)

        self.colrel_model.load_state_dict(torch.load(colrel_model_path,
                                                     map_location=self.device))
    ...
    def annotate_columns(self,
        ...
        outputs = self.coltype_model.bert.encoder(self.coltype_model.bert.embeddings(batch["data"].T),
                                                  output_attentions=True,
                                                  output_hidden_states=True)
        hidden_states = outputs[1] # 0: word embeddings, -1: last_hidden_states
        last_hidden_states = outputs.last_hidden_state.squeeze(0) # SeqLen * DimSize

        cls_indexes = torch.nonzero(
            batch["data"].T.squeeze(0) == self.tokenizer.cls_token_id).T.squeeze(0).detach().cpu().numpy()

        emb_list = []
        for cls_id, cls_index in zip(batch["label"].detach().cpu().numpy(), cls_indexes):
        ...
        #first_attentions = outputs.attentions[0]
        #first_attentions.sum([0, 1])
        ...
        cls_indexes = torch.nonzero(batch["data"].T == self.tokenizer.cls_token_id)
        ...
        # ts_pred = (filtered_logits >= math.log(0.5)).int().detach().cpu().tolist()
        coltype_pred = filtered_logits.argmax(1)
        coltype_pred_labels = [self.coltype_mlb.classes_[x] for x in coltype_pred]
        ...
        #ts_pred = all_preds[idxes, :].tolist()

        colrel_pred = filtered_logits.argmax(1)
        colrel_pred = colrel_pred[1:] # Drop the first prediction
        colrel_pred_labels = [self.colrel_mlb.classes_[x] for x in colrel_pred]
...
class Dosolo:
    def __init__(self,
        ...
        ## Load models
        coltype_model_path = os.path.join(basedir, "model/turl_single_bert_bert-base-uncased-bs16-ml-16__turl-1.00_best_micro_f1.pt")
        ...
        coltype_config = BertConfig.from_pretrained(self.args.shortcut_name,
                                                    ...
                                                    output_attentions=True,
                                                    output_hidden_states=True)
        self.coltype_model = BertForSequenceClassification(coltype_config).to(self.device)
        self.coltype_model.load_state_dict(torch.load(coltype_model_path,
                                                      map_location=self.device))
        self.coltype_model.eval()
...
if __name__ == "__main__":
    ...
    if args.model == "viznet":
        raise NotImplementedError("Viznet model not supported yet.")
