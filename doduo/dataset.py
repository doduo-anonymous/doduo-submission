    ...
    def __init__(self,
        ...
        if split in ["train", "valid"]:
            ...
            for i in range(5):
                if i == cv:
                    continue
                filepath = os.path.join(base_dirpath,
                                        basename.format(i))
        ...
        self.df["data_tensor"] = self.df["data"].apply(
            lambda x: torch.LongTensor(tokenizer.encode(x,
                                                        add_special_tokens=True,
                                                        max_length=max_length + 2)).to(device))
        self.df["label_tensor"] = self.df["class_id"].apply(
            lambda x: torch.LongTensor([x]).to(device))  # Can we reduce the size?
...
class SatoCVTablewiseDataset(Dataset):
    def __init__(self,
        ...
        for i, (index, group_df) in enumerate(df.groupby("table_id")):
            ...
            class_ids = torch.LongTensor(group_df["class_id"].values).to(device)
        ...
        for i, (index, group_df) in enumerate(self.df.groupby("table_id")):
            ...
            for j, (_, row) in enumerate(group_df.iterrows()):
                ...
                row["data_tensor"] = torch.LongTensor(
                    tokenizer.encode(
                        group_df.iloc[0]["data"],
                        add_special_tokens=True,
                        max_length=max_length + 2) + tokenizer.encode(
                        row["data"],
                        add_special_tokens=True,
                        max_length=max_length + 2)).to(device)
